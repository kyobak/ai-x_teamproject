...
- **2026-09-19 (Claude, 6차)**: 영어 UI 전면 적용 — 화면 문구 `useL()` 로 한/영 병기, 서버 데이터는 `lib/dataI18n.ts` 사전(식당·매장·메뉴·건물·운영시간·오류 메시지), 숫자가 든 문장(세탁 남은 시간, 셔틀 탑승 안내)은 화면에서 생성. 영어 화면에서 내 정보 저장 시 한국어로 되돌아가던 버그 수정. 푸시 알림 한/영 병기. Android SDK 라이선스 동의(사용자 승인) 후 APK 빌드 성공 → `apps/web/public/erica-wait.apk` 로 배포, 푸터에 다운로드 링크.
