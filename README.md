...
| Should. 셔틀 | 정류장 2곳(셔틀콕→한대앞역, 한대앞역→셔틀콕·창의인재원), 실제 시간표. 하냥이 줄 애니메이션 + 다음 차 남은 분 + "지금 줄 서면 몇 시 차" | ✅ 동작 |
| Could. 오픈스페이스 | 8곳(융합교육관 2·경상관 상휴·솔성관 꿈의 둥지·제1공학관 해동학술정보실·제1과기관 북카페·체육관·디자인라운지, 수용 인원 포함). 카메라 실내 계수(앉은 사람 포함) → 재실 인원 | ✅ 동작 |
| 계정 | 닉네임+비밀번호 로그인, 내 정보(기숙사·자주 가는 식당·기본 정류장·알림·언어) | ✅ 동작 (학번·실명 없음) |
| 데모 모드 | 실측·제보가 없는 자원은 시연용 시뮬레이션 값으로 채움 (`DEMO_MODE=1`, source="demo" 로 표시) | ✅ |
| 배포 | Fly.io/Render + Vercel 설정, CI, 가이드 (`docs/deploy.md`) | ✅ 파일 준비 (계정 연결은 팀이) |
...
### 화면 구조 (분야 → 세부 → 상세)
```
/                 홈: 가장 빠른 학식 히어로 + 4개 분야 타일
/laundry          세탁·건조: 인재관 / 창의관 / 행복관   → /laundry/[building] 기기 목록·대기열
/space            오픈스페이스 8곳 (여유 순)             → /space/[id] 재실 인원·수용 인원
/shuttle          정류장 2곳, 하냥이 줄 애니메이션        → /shuttle/[id] 남은 시간·탑승 예측·시간표
/cafeteria        구내식당 4 + 푸드코트 2 (대기 짧은 순) → /cafeteria/[id] 계산식·메뉴·매장
/login /me        닉네임 계정 · 내 정보
...
.venv/bin/python vision/run_video.py --zone vision/zones/multi-demo.json --show   # 한 영상에서 학식 줄+셔틀 줄+실내 동시
```
구역 종류 `queue`(줄: 인원·처리율·체류시간) / `room`(실내: 재실 인원). 체류시간은 추적 ID 가 구역에 머문 평균 초로, 대기시간 추정의 검증값이 됩니다.

**앉은 사람 계수(room 구역)**: 발이 책상에 가려지므로 박스 중심으로 판정, 신뢰도 하한 0.2(`--room-conf`), 입력 해상도 960(`--imgsz`), 넓은 라운지는 `--tile`(화면을 나눠 검출). 더 정확하게는 `--model yolo11s.pt`.
