...
export default function LaundryPage() {
  ...
  return (
    <>
      <Header title={t("nav.laundry")} back="/" />
      <main className="space-y-3 p-4">
        <PwaSetup />
        <p className="text-xs text-muted">{zone} · 앱은 안내 도구이며 실제로 먼저 온 사람이 우선입니다. 기기가 비면 1순위에게 알려드립니다.</p>
        {machines.map((m) => (
          <MachineCard key={m.id} r={m} deviceId={deviceId} onChanged={refreshTickets} ticket={myTickets.find((tk) => tk.resource_id === m.id)} />
        ))}
        {machines.length === 0 && <p className="text-sm text-muted">기기 정보를 불러오는 중…</p>}
