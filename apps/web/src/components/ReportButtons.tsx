...
export function ReportButtons({ resourceId, deviceId }: { resourceId: string; deviceId: string }) {
  ...
  const send = async (level: number) => {
    ...
    try {
      ...
      setMsg(L("제보 감사합니다. 30분 동안 반영됩니다.", "Thanks! Your report counts for 30 minutes."));
    } catch (e) {
      try {
        const d = JSON.parse((e as Error).message) as { message: string; retry_at: string };
        const at = new Date(d.retry_at).toLocaleTimeString(lang === "en" ? "en-US" : "ko-KR", { hour: "2-digit", minute: "2-digit" });
        setMsg(`${D(d.message)} ${L(`(${at} 이후 가능)`, `(try again after ${at})`)}`);
      } catch { setMsg(D((e as Error).message)); }
    }
  };
  return (
    <div>
