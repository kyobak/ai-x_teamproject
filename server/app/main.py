...
from app.routers import admin, auth, laundry, push, reports, resources, stream, vision
    ...
    while True:
        try:
            ...
            with get_conn() as conn:
                ...
                for r in conn.execute("SELECT id FROM resources WHERE kind IN ('cafeteria','shuttle','space')").fetchall():
                    publish_resource(conn, r["id"])
...
for r in (resources.router, vision.router, laundry.router, reports.router, admin.router, stream.router, push.router, auth.router):
